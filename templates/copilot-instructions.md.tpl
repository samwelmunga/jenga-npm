...
- Each **skill** is a self-contained instruction set stored under `skills/<skill-name>/`.
...
This script sources `lib/resolve-project-dir.sh` and delegates to `hooks/on_session_end.sh`, which handles queue routing, rapport detection, handoff file processing, and todo cleanup. See [`docs/hook-parity.md`](docs/hook-parity.md) for the full Claude Code ↔ Copilot hook parity reference.

### Notes

- Always resolve file paths relative to `JENGA_PROJECT_DIR`.
- When a skill asks you to read a file such as `SKILL.md` or a task file, look for it inside `JENGA_PROJECT_DIR/skills/` or `JENGA_PROJECT_DIR/project/board/` respectively.
