...
| `j.playbook-new` | Guided wizard that walks you through authoring a new project-local playbook (`project/.playbooks/<id>.json`) — id, name, description, keywords, examples, and an ordered list of skills, each validated against the real playbook and skill catalogs, self-validated via `load-playbooks.sh lookup` before success is ever reported. |
...
| `j.uncharted` | Entry point for code with no board provenance — four modes: `segment` (a file or directory), `import` (an external source), `onboard` (a whole pre-existing codebase), `refresh` (incremental re-scan of an already-onboarded codebase since its last baseline, skipping unchanged subsystems). |
