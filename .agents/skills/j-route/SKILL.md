---
name: j:j-route
description: Polyfill alias of the route skill under a collision-safe directory name. Identical behavior to /route — Intelligently routes a prompt to the best-matching skill in the project. Reads available skills, matches the input semantically and by keyword, enriches the prompt with board context and documentation references, then invokes the matched skill with the full context prepended. Use when the bare /route form is shadowed by another tool's own built-in command of the same name.
keywords:
  - "route"
  - "dispatch"
  - "auto skill"
...
This skill is a literal-directory-name duplicate of `skills/route/`. It exists so that `/j-route` (and `j:j-route`) give a guaranteed-unshadowed way to reach the same flow as `/route`, even if a host tool's own built-in command of the same name would otherwise shadow or override the bare `/route` alias (Claude Code's native skill resolution is a literal-string, directory-name-based match — see `docs/skill-authoring.md`'s "Invocation Convention").

This file is generated/synced by `scripts/generate-j-alias.sh route` from `skills/route/SKILL.md` — do not hand-edit it; re-run the generator instead to pick up source changes.
