---
name: j:j-train
description: Polyfill alias of the train skill under a collision-safe directory name. Identical behavior to /train — Scaffold and run ML training jobs. Use `new <type> <job-name>` to scaffold a job from a template, or `run <job-dir>` to execute the two-phase validate → train pipeline on an existing job. Use when the bare /train form is shadowed by another tool's own built-in command of the same name.
keywords:
  - train
  - ml training
  - model training
...
This skill is a literal-directory-name duplicate of `skills/train/`. It exists so that `/j-train` (and `j:j-train`) give a guaranteed-unshadowed way to reach the same flow as `/train`, even if a host tool's own built-in command of the same name would otherwise shadow or override the bare `/train` alias (Claude Code's native skill resolution is a literal-string, directory-name-based match — see `docs/skill-authoring.md`'s "Invocation Convention").

This file is generated/synced by `scripts/generate-j-alias.sh train` from `skills/train/SKILL.md` — do not hand-edit it; re-run the generator instead to pick up source changes.

> **Not part of the public npm package.** `/train` is intentionally excluded from the public `@jenga-ai/agent` package (via `.publicignore`, since 2026-08-16 — predating the first npm publish) because it is planned to ship as its own standalone package in the future.

## Usage

```
python skills/j-train/train_cli.py <subcommand> [args]
