...
This skill is a literal-directory-name duplicate of `skills/jenga/`. It exists so that `/j-jenga` (and `j:j-jenga`) give a guaranteed-unshadowed way to reach the same flow as `/jenga`, even if a host tool's own built-in command of the same name would otherwise shadow or override the bare `/jenga` alias (Claude Code's native skill resolution is a literal-string, directory-name-based match — see `docs/skill-authoring.md`'s "Invocation Convention").

This file is generated/synced by `scripts/generate-j-alias.sh jenga` from `skills/jenga/SKILL.md` — do not hand-edit it; re-run the generator instead to pick up source changes.
...
1. Invoke `skills/j-jenga/scripts/render-picker.sh` with no arguments (start mode). Relay its STDOUT (the numbered checklist) to the user verbatim — no summarizing, no reformatting. Capture the `STATE_FILE:` path from STDERR.
2. Wait for the user's chat reply, then invoke `skills/j-jenga/scripts/render-picker.sh <state_file> "<raw_reply>"` (continue mode).
...
1. Invoke `skills/j-jenga/scripts/resolve-id.sh "<raw argument>"` directly — the picker is skipped entirely in this branch.
...
1. Invoke `skills/j-jenga/scripts/render-confirmation.sh "<comma-separated resolved ids from whichever branch above>"` (start mode). Relay STDOUT (the confirmation tree) to the user verbatim. Capture the `STATE_FILE:` path from STDERR.
2. Wait for the user's chat reply, then invoke `skills/j-jenga/scripts/render-confirmation.sh <state_file> "<raw_reply>"` (continue mode).
