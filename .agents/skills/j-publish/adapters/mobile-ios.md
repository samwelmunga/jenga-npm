...
`skills/j-publish/scripts/ios_pipeline.sh`.
...
```bash
bash skills/j-publish/scripts/validate_ios_env.sh <path-to-publish.json>
```

### Pipeline entrypoint

```bash
bash skills/j-publish/scripts/ios_pipeline.sh <target> <path-to-publish.json> [--dry-run] [--non-interactive]
```
