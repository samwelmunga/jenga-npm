...
This skill is a literal-directory-name duplicate of `skills/distribute/`. It exists so that `/j-distribute` (and `j:j-distribute`) give a guaranteed-unshadowed way to reach the same flow as `/distribute`, even if a host tool's own built-in command of the same name would otherwise shadow or override the bare `/distribute` alias (Claude Code's native skill resolution is a literal-string, directory-name-based match — see `docs/skill-authoring.md`'s "Invocation Convention").

This file is generated/synced by `scripts/generate-j-alias.sh distribute` from `skills/distribute/SKILL.md` — do not hand-edit it; re-run the generator instead to pick up source changes.
...
- If release type is `amend`: run `bash skills/j-distribute/scripts/check-version.sh <path>`.
...
```bash
bash skills/j-distribute/scripts/distribute-changes.sh <project_path> --dry-run
```
...
```bash
bash skills/j-distribute/scripts/distribute-changes.sh <project_path>
```
...
```bash
bash skills/j-distribute/scripts/commit-version-bump.sh
```
...
| Script | Purpose |
|--------|---------|
| `skills/j-distribute/scripts/distribute-changes.sh <path> [--dry-run]` | Copy framework files to a single consuming project |
| `skills/j-distribute/scripts/commit-version-bump.sh` | Commit the `package.json` version bump |
| `skills/j-distribute/scripts/check-version.sh <path>` | Amend mode: exits 0 if target needs update, 1 if up to date, 2 if invalid |

## Config

`distribute.config.json` at the repo root is the registry of consuming projects.

Schema reference: `skills/j-distribute/CONFIG_SCHEMA.md`
