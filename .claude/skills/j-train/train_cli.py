...
def run_wizard(full=False):
    ...
    try:
        ...
        print(f"   Run: python skills/j-train/train_cli.py run jobs/{scaffold_dir.name}\n")
...
def _print_next_steps(dest, job_name):
    ...
    if workflow.get("generate_start_sh", True):
        ...
        print(f"      or:  python skills/j-train/train_cli.py run {dest.relative_to(REPO_ROOT)}")
    else:
        print(f"   3. Run:  python skills/j-train/train_cli.py run {dest.relative_to(REPO_ROOT)}")
