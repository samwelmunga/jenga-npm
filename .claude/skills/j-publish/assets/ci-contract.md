...
2. Run `bash skills/j-publish/scripts/validate_config.sh <resolved-path>`
3. Abort immediately with exit code `4` if the file is missing, invalid JSON, or fails validation
4. For the iOS adapter, run `bash skills/j-publish/scripts/validate_ios_env.sh <resolved-path>` before build/upload work begins
