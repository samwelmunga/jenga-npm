---
name: j:j-strategy
description: Polyfill alias of the strategy skill under a collision-safe directory name. Identical behavior to /strategy — Walk through a guided conversation to capture or update docs/STRATEGY.md — covering Vision, Value Proposition, Scope, and Target Audience — one section at a time. Use when the bare /strategy form is shadowed by another tool's own built-in command of the same name.
metadata:
...
This skill is a literal-directory-name duplicate of `skills/strategy/`. It exists so that `/j-strategy` (and `j:j-strategy`) give a guaranteed-unshadowed way to reach the same flow as `/strategy`, even if a host tool's own built-in command of the same name would otherwise shadow or override the bare `/strategy` alias (Claude Code's native skill resolution is a literal-string, directory-name-based match — see `docs/skill-authoring.md`'s "Invocation Convention").

This file is generated/synced by `scripts/generate-j-alias.sh strategy` from `skills/strategy/SKILL.md` — do not hand-edit it; re-run the generator instead to pick up source changes.
