...
`skills/j-publish/scripts/npm_pipeline.sh`.
...
```bash
bash skills/j-publish/scripts/validate_npm_env.sh <path-to-publish.json>
```

### Pipeline entrypoint

```bash
bash skills/j-publish/scripts/npm_pipeline.sh <target> <path-to-publish.json> [--dry-run] [--non-interactive]
```
...
becomes visible on the registry. See `skills/j-publish/SKILL.md`'s
